...
struct Cli {
    ...
    /// SNAT csv file path
    #[arg(short, long, default_value = "snat.csv")]
    snat_filename: PathBuf,
...
enum Commands {
    ...
    /// Creates DNAT csv file
    #[command(name = "snat-init")]
    SNatInit,

    /// Generate NAT config
    #[command(name = "snat-gen")]
    SNatGen,
...
fn main() -> Result<()> {
    ...
    match &cli.command {
        ...
        Some(Commands::SNatInit) => {
            let mut wtr = csv::WriterBuilder::new()
                .flexible(true)
                .from_path(cli.snat_filename.clone())
                .context(format!(
                    "Failed to write csv to {}",
                    cli.snat_filename.display()
                ))?;
            wtr.serialize(SNatRecord::Simple(SimpleSNat {
                src_ip: IpAddr::V4(Ipv4Addr::new(10, 10, 10, 10)),
                protocol: Some("tcp".to_owned()),
                rewrite_ip: IpAddr::V4(Ipv4Addr::new(20, 20, 20, 20)),
                comment: "this nat".to_owned(),
                host_fib: "rtr".to_owned(),
            }))?;
            wtr.serialize(SNatRecord::Custom(CustomSNat {
                custom_cmd: "add ....".to_owned(),
                comment: "this nat".to_owned(),
                host_fib: "rtr".to_owned(),
            }))?;
            println!(
                "{} was created.",
                cli.snat_filename.to_str().context("filename error")?
            );
        ...
        Some(Commands::SNatGen) => {
            ...
            let mut nat_records: Vec<SNatRecord> = Vec::new(); //rdr.deserialize().collect::<Result<Vec<_>, _>>()?;

            for r in rdr.records() {
                let r = r?;
                if r[1].starts_with("add ") {
                    let r: CustomSNat = r.deserialize(None)?;
                    nat_records.push(SNatRecord::Custom(r));
                } else {
                    let r: SimpleSNat = r.deserialize(None)?;
                    nat_records.push(SNatRecord::Simple(r));
                }
            }
            ...
            for r in &records {
                configs.insert(
                    r.name.clone(),
                    format!(
                        "# {} SNAT config generated by mt-wg-meshconf at {}",
                        r.name,
                        chrono::Local::now().format("%Y-%m-%d %H:%M:%S")
                    ),
                );
                ...
                nat_records.iter().for_each(|n| {
                    ...
                    match n {
                        SNatRecord::Simple(simple_nat) => {
                            ...
                            s.push_str(&format!(
                                "add action=mark-routing chain=prerouting dst-address-list=!LAN src-address={} new-routing-mark={}",
                                simple_nat.src_ip, simple_nat.host_fib
                            ));
                            if let Some(protocol) = &simple_nat.protocol {
                                s.push_str(&format!(" protocol={protocol}"));
                            }
